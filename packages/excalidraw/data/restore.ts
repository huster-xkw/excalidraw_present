...
export const restoreAppState = (
  ...
  return {
    ...
    editingFrame: null,
    // ensure pages are valid — if loaded from an old file without pages,
    // the default page will have empty elements; the caller (restore flow)
    // is responsible for populating the current page's elements from the
    // top-level elements array.
    pages: nextAppState.pages?.length
      ? nextAppState.pages
      : defaultAppState.pages,
    currentPageId: nextAppState.currentPageId || defaultAppState.currentPageId,
  };
};
