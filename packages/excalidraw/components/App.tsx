...
class App extends React.Component<AppProps, AppState> {
                  ...
                  <ExcalidrawAppStateContext.Provider value={this.state}>
                    ...
                    >
                      ...
                      >
                        ...
                        <PageFlipOverlay
                          direction={this.state.pageFlipDirection}
                          onAnimationEnd={() => {
                            this.setState({ pageFlipDirection: null });
                          }}
                        />
  ...
  private initializeScene = async () => {
    ...
    // Sync pages with elements:
    // - Old files without pages: wrap restored elements into the default page
    // - New files with pages: load current page's elements into the scene
    let elementsToLoad = restoredElements;
    if (
      restoredAppState.pages &&
      restoredAppState.pages.length > 0 &&
      restoredAppState.pages[0].elements.length > 0
    ) {
      // New file with pages — load current page's elements
      const currentPage = restoredAppState.pages.find(
        (p: any) => p.id === restoredAppState.currentPageId,
      );
      if (currentPage) {
        elementsToLoad = restoreElements(currentPage.elements, null, {
          repairBindings: true,
          deleteInvisibleElements: true,
        });
      }
    } else {
      // Old file or fresh start — sync elements into the default page
      restoredAppState = {
        ...restoredAppState,
        pages: restoredAppState.pages.map((page: any, idx: number) =>
          idx === 0 ? { ...page, elements: [...restoredElements] } : page,
        ),
      };
    }
    ...
    if (initialData?.scrollToContent) {
      restoredAppState = {
        ...restoredAppState,
        ...calculateScrollCenter(elementsToLoad, {
          ...restoredAppState,
          width: this.state.width,
          height: this.state.height,
          offsetTop: this.state.offsetTop,
